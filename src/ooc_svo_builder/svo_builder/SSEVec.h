...
static inline const T operator ^ (const Vec<D,T> &v1, const Vec<D,T> &v2)
{
    ...
    if (typeid(T) == typeid(float)){
        ...
        if (D == 4){
            ...
            fin = _mm_dp_ps ( X, Y, 0x31);

        }
        _mm_storeu_ps(&ret[0], fin);
        //assert( std::fabs(sum - ret[0]) < 1e-6);
        return ret[0];
    }
    else{
        using namespace ::std;
...
inline Vec<4,float> CrossProduct(Vec<4,float> a, Vec<4,float> b)
{
    __m128 rA, rB, fin;
   rA = _mm_loadu_ps(&a[0]);
   rB = _mm_loadu_ps(&b[0]);
  ...
  fin = _mm_sub_ps(
    _mm_mul_ps(_mm_shuffle_ps(rA, rA, _MM_SHUFFLE(3, 0, 2, 1)), _mm_shuffle_ps(rB, rB, _MM_SHUFFLE(3, 1, 0, 2))),
    _mm_mul_ps(_mm_shuffle_ps(rA, rA, _MM_SHUFFLE(3, 1, 0, 2)), _mm_shuffle_ps(rB, rB, _MM_SHUFFLE(3, 0, 2, 1)))
  );

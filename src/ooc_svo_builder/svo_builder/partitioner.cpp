...
void createBuffers(const TriInfo& tri_info, const size_t n_partitions, const size_t gridsize, vector<Buffer*> &buffers){
	...
	for (size_t i = 0; i < n_partitions; i++){
		...
		mortonDecode_magicbits(morton_part*i, bbox_grid.min[2], bbox_grid.min[1], bbox_grid.min[0]);
		mortonDecode_magicbits((morton_part*(i + 1)) - 1, bbox_grid.max[2], bbox_grid.max[1], bbox_grid.max[0]); // -1, because z-curve skips to first block of next partition

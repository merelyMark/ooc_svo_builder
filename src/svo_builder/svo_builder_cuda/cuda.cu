...
__device__
void voxelize_triangle(float3 v0, float3 v1, float3 v2,const uint64 morton_start,
                       const uint64 morton_end, const float unitlength, voxel_t* voxels,
                       uint64 *data, float sparseness_limit, uint *nfilled,
                       const uint3 &p_bbox_grid_min, const uint3 &p_bbox_grid_max,
                       const float unit_div, const float3 &delta_p,	size_t data_max_items)
...
template<bool COUNT_ONLY>
__global__
void voxelize(const float3 *v0, const float3 *v1, const float3 *v2, const uint *tri_idx,
              const uint64 morton_start, const uint64 morton_end, const float unitlength,
              voxel_t* voxels, uint64 *data, float sparseness_limit, uint *nfilled,
               const uint3 p_bbox_grid_min, const uint3 p_bbox_grid_max, const float unit_div,
              const float3 delta_p,	size_t data_max_items, size_t num_triangles)
{
...
#include <tbb/tbb.h>
extern "C"
void cudaCount(const float3* d_v0, const float3*d_v1, const float3*d_v2, uint *d_tri_idx, uint *d_nfilled, uint partition_idx, voxel_t *d_voxels, uint64 *d_data,
             ...
             const uint3 &p_bbox_grid_min, const uint3 &p_bbox_grid_max, const float unit_div, const float3 &delta_p,	size_t data_max_items, uint num_triangles)
{
    ...
    //get count
    voxelize<true><<<5000,64>>>(d_v0, d_v1, d_v2, d_tri_idx, morton_start, morton_end, unitlength, d_voxels, d_data, use_data, d_nfilled+partition_idx,
             ...
             uint idx)
{
    ...
    //fill d_data
    voxelize<false><<<5000,64>>>(d_v0, d_v1, d_v2, d_tri_idx, morton_start, morton_end, unitlength, d_voxels, d_data + idx, use_data, d_nfilled+partition_idx,
                    p_bbox_grid_min, p_bbox_grid_max, unit_div, delta_p, data_max_items, num_triangles);
...
extern "C"
uint cudaCountFinalize(uint nfilled_blocks, uint *d_nfilled)
{
    thrust::device_ptr<uint> d = thrust::device_pointer_cast(d_nfilled);
    uint ret = thrust::reduce(d, d+nfilled_blocks);
    thrust::inclusive_scan(d, d+nfilled_blocks, d);
    return ret;
}
...
{
    thrust::device_ptr<uint64> dptr = thrust::device_pointer_cast(d_data);

    thrust::sort(dptr, dptr+cnt);

    thrust::device_ptr<uint64> new_end = thrust::unique(dptr, dptr+cnt);

    uint ret = thrust::raw_pointer_cast(new_end) - d_data;
    return ret;
}
